...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        {
            //Inserts the email and name datas from the page into the database (in this case its website_users)
            // $sql = "INSERT INTO website_users (email, name)
            //         VALUES ('$email', '$name')";
            //Once the box is empty it tells you you are registered or if the email is already taken -w- the unique tag in the database
            if(!empty($email))
            {
                try
                {
...
"You’re the only you — and that’s your power.",
    ...
    ini_set("SMTP", "smtp.hackclub.app"); 
    ini_set("smtp_port", "587"); 
    ini_set("sendmail_from", "hobbes@hackclub.app");
    ...
    $to = $email; 
    $subject = "Positive Phrase of the Day";
    $message = $name . " , " . $randomString;
    $headers = "From: hobbes@hackclub.app";
